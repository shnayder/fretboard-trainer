 ...
 * @returns {Element}
 */
export function pickCalibrationButton(buttons, prevBtn) {
...
function runCalibration(opts) {
  ...
  function startTrial() {
    ...
    // For multi-target trials, count warmup by trial (not by press)
    const warmupPresses = isSearchMode() ? 0 : WARMUP_TRIALS;

    if (trialIndex >= TRIAL_COUNT) {
      cleanup();
      // For search mode, warmup is counted differently: skip all presses
      // from the first WARMUP_TRIALS trials
      const median = isSearchMode()
        ? computeMedian(times.slice(warmupPresses))
        : computeMedian(times.slice(WARMUP_TRIALS));
      onComplete(median);
      return;
    }
