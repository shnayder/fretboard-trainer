...
- **File headers**: purpose and key constraints. See `quiz-engine-state.ts` for
  ...
  ```
- **JSDoc** on exported functions — prose explaining *why* or edge cases. Omit
...
4. **Interfaces for shared shapes, inline for local ones.** Shared types live
   in `src/types.ts`. A type used in only one file stays inline.

5. **Non-null assertions (`!`) for build-time DOM.** Elements generated by
   `html-helpers.ts` are known to exist. Use `!` after `getElementById`, not
   a runtime `if` check.
...
8. **Replace `@param`/`@returns` JSDoc with types.** Keep prose JSDoc that
   explains *why* or *edge cases*. Delete JSDoc that only restates types.
