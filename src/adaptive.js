...
export function createAdaptiveSelector(
   ...
   * i.e. answering at or below the automaticity target) but at least one
   * item's recall has since decayed below threshold. This avoids false
   * positives from items still being learned or one-off correct answers.
   */
  function checkNeedsReview(items) {
    ...
    for (const id of items) {
      ...
      if (!stats || stats.lastCorrectAt == null) return false;
      const speed = computeSpeedScore(stats.ewma, cfg);
